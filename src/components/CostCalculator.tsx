...
export default function CostCalculator({ onOpenBookingFormWithCalc }: CostCalculatorProps) {
  ...
  return (
    <section className="py-16 sm:py-28 bg-white dark:bg-[#0B0514] relative overflow-hidden transition-colors duration-300">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        
        <div className="bg-gradient-to-br from-[#4B2476] via-[#351859] to-[#200D3A] dark:from-[#250F45] dark:via-[#1A0A33] dark:to-[#100524] rounded-3xl p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden border border-purple-800 dark:border-purple-700/60">
          ...
          <div className="space-y-6 relative z-10 max-w-4xl mx-auto">
            ...
            {/* 1. Pilih Durasi Hari */}
            <div className="bg-white/10 dark:bg-purple-950/70 backdrop-blur-md p-5 rounded-2xl border border-white/15 dark:border-purple-700/50 space-y-3">
                ...
                {durationOptions.map((dur) => (
                  <button
                    ...
                    className={`py-3 px-3 rounded-xl text-xs font-bold transition-all text-center flex items-center justify-center gap-1.5 border ${
                      duration === dur
                        ? 'bg-[#F5B027] text-gray-950 border-[#F5B027] shadow-md font-extrabold'
                        : 'bg-white/10 dark:bg-purple-900/40 text-white border-white/15 dark:border-purple-700/40 hover:bg-white/20'
                    }`}
                  >
                    {duration === dur && <Check className="w-3.5 h-3.5 text-gray-950" />}
                    <span>{dur}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {hotelOptions.map((htl) => (
                  ...
                  >
                    ...
                    {hotelChoice === htl && <Check className="w-3.5 h-3.5 text-gray-950 shrink-0 ml-1" />}
                  </button>
                ))}
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                {airlineOptions.map((air) => (
                  ...
                  >
                    ...
                    {airlineChoice === air && <Check className="w-3.5 h-3.5 text-gray-950 shrink-0 ml-1" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white/10 dark:bg-purple-950/70 backdrop-blur-md p-5 rounded-2xl border border-white/15 dark:border-purple-700/50 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-extrabold text-white bg-white/20 dark:bg-purple-900/60 px-3 py-1 rounded-full border border-white/10">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-11 h-11 rounded-xl bg-white/20 dark:bg-purple-900/60 hover:bg-white/30 font-bold text-xl text-white flex items-center justify-center transition-colors border border-white/10"
                >
                  -
                </button>
                <input
            ...
            <div className="pt-4 text-center">
              <button
                ...
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#F5B027] hover:bg-amber-400 text-gray-950 font-extrabold text-sm sm:text-base px-8 py-4 rounded-2xl shadow-xl transition-all"
              >
