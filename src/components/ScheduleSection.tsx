...
import { Calendar, Plane, CheckCircle2, PhoneCall } from 'lucide-react';
...
export default function ScheduleSection() {
  ...
  return (
    <section id="schedule" className="py-20 sm:py-32 bg-white dark:bg-[#0F0F14] relative transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">
            Jadwal Keberangkatan <span className="text-[#4B2476] dark:text-[#F5B027]">Umrah 1448 H / 2026-2027</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {schedules.map((sch, idx) => (
            <div
              key={idx}
              className="bg-[#FAFAFD] dark:bg-slate-900 rounded-2xl p-6 border border-gray-200 dark:border-slate-800 shadow-2xs hover:shadow-md transition-shadow flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between border-b border-gray-200 dark:border-slate-800 pb-4 mb-4">
                  <div className="flex items-center gap-2 text-[#4B2476] dark:text-[#F5B027] font-bold text-lg">
                    <Calendar className="w-5 h-5 text-[#F5B027]" />
                    <span>{sch.date}</span>
                  </div>
                  <span className="text-xs font-bold bg-purple-100 dark:bg-purple-950 text-[#4B2476] dark:text-purple-300 px-3 py-1 rounded-full">
                    {sch.status}
                  </span>
                </div>

                <h3 className="font-extrabold text-gray-900 dark:text-white text-lg mb-3">{sch.program}</h3>
                ...
                <div className="space-y-1 mb-6">
                  <div className="flex justify-between text-xs font-semibold">
                    <span className="text-gray-500 dark:text-gray-400">Kuota Terisi</span>
                    <span className="text-[#4B2476] dark:text-[#F5B027] font-bold">
